...
def busca_tabu(cities, initial=0):
    current = initial
    total_distance = 0
    BTmax = 20

    solucaoinicial = greedy_closest_neighbor(cities)
    S2 = solucaoinicial
    lista_tabu = []
    S = solucaoinicial
    # Pega a linha da matriz que representa os vizinhos da cidade atual (current)
    ...
    while iter - melhor_iter <= BTmax:
        vizinhos = cities[current]

        iter = iter + 1
        for idx in range(len(vizinhos)):

            distance = vizinhos[idx]
            if idx not in lista_tabu and distance > 0 and distance < best_distance:
                new_distance = total_distance + best_distance
                best_neighbour = idx
                best_distance = distance
                lista_tabu.append(best_neighbour)
                S.append(best_neighbour)
    ...
    return (total_distance, S2)


print(busca_tabu(cities))